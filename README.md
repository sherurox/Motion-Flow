...
Motion-Flow is inspired by and recreated from the excellent open-source project sign.mt by Amit Moryossef. I rebuilt the in-browser translation pipeline to learn the architecture end-to-end and contributed my own work in [list your specific additions — e.g., the benchmarking suite, caching layer, deployment setup]. Full credit for the original concept and foundational approach goes to the sign.mt authors and the underlying open-source tools (BrowserMT/Bergamot, MediaPipe, Sutton SignWriting).
